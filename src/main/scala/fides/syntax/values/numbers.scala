...
/**
  ...
  */
final case class Nat[B <: Bits] @publicInBinary private(bits: B) extends NtrlC[NatT[B]]:
...
/**
  ...
  */
final case class Add[M <: TopM](terms: Code2[Expr2[CollectedUT[NatUT]], M]) extends Code2[Expr2[NatUT], M]

/**
  * Outputs the product of the inputs.
  */
final case class Multiply[M <: TopM](factors: Code2[Expr[CollectedUT[NatUT]], M]) extends Code2[Expr2[NatUT], M]

/**
  * Outputs -1 if the lhs is larger, 0 if they are equal, 1 if the rhs is larger.
  */
final case class Compare[M <: TopM](
  lhs: Code2[Expr2[NatUT], M],
  rhs: Code2[Expr2[NatUT], M],
) extends Code2[Expr2[NatUT], M]
