...
import util.{BoolT, BotB, FlseT, MultisetOps, TopB, TrueT}

/**
  * Used for multisets of pieces of code, at the syntax level.
  */
transparent sealed trait Args[S <: TopS, I <: BoolT, Q <: TopQ] extends ConsM[ArgsS[I, S], Q]:
...
object Args:
  ...
  type None = ConsM[ArgsS[FlseT, OffBotS], BotQ]
  type Some[S <: TopS, Q <: TopQ] = ConsM[ArgsS[TrueT, S], Q]

  final case class Matcher[S <: TopS, Q <: TopQ](
    typeRepr: ConsM[S, Q],
  ) extends ConsM[ArgsS[BoolT, S], Q | ConsQ[Polarity[TopB, BotB, BotB], BotQ]]
  // todo add another type parameter I?

  private case object Empty extends Args[OffBotS, FlseT, BotQ], None:
    def arguments: Multiset[ConsM[OffBotS, BotQ]] = summon[MultisetOps[Multiset]].empty
  private final class NonEmpty[S <: TopS, Q <: TopQ](
    first: ConsM[S, Q],
    others: ConsM[S, Q]*,
  ) extends Args[S, TrueT, Q], Some[S, Q]:
...
/**
  ...
  */
final case class Zip[I <: BoolT, S <: TopS, P <: TopP, Q <: TopQ](
  pieces: ConsM[Polar2S[CollectedD[I, QuoteD[S]], P], Q],
) extends ConsM[Polar2S[QuoteD[ArgsS[I, S]], P], Q]
