...
final case class Forward[K <: N, T <: V[K]](inp: E[K, T], out: Multiset[Out[K, T]]) extends P[K]
final case class Wait[K <: N, T <: V[K]](token: E[K, U], inp: E[K, T], out: Out[K, T]) extends P[K]

// Could be an agent instead of a primitive...
final case class Match[K <: N](inp: E[K, Code[K]], pattern: Patt, noMatch: Out[K, Code[K]]) extends P[K]
...
final case class Swappable[K <: N](inp: E[K, Code[K]], process: P[K]) extends P[K]
final case class Annotated[K <: N](process: P[K], annotation: P[_ <: N]) extends P[K]
final case class Guarded[K <: N](handler: Out[K, Error], process: P[K]) extends P[K]
